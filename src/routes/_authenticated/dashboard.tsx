...
export const Route = createFileRoute("/_authenticated/dashboard")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Farm Dashboard — AgriSense AI" },
      { property: "og:description", content: "Live soil moisture, plant health and alert overview." },
    ],
  }),
  component: Dashboard,
});
...
const PANELS = [
  { to: "/water", emoji: "💧", icon: Droplets, titleKey: "panelWaterTitle", textKey: "panelWaterText" },
  { to: "/disease", emoji: "🔬", icon: ScanLine, titleKey: "panelDiseaseTitle", textKey: "panelDiseaseText" },
  { to: "/live", emoji: "📷", icon: Camera, titleKey: "panelLiveTitle", textKey: "panelLiveText" },
  { to: "/farm-3d", emoji: "🌿", icon: Boxes, titleKey: "panelFarm3dTitle", textKey: "panelFarm3dText" },
  { to: "/hotspots", emoji: "🗺️", icon: MapPin, titleKey: "panelHotspotsTitle", textKey: "panelHotspotsText" },
  { to: "/pests", emoji: "🐛", icon: Bug, titleKey: "panelPestsTitle", textKey: "panelPestsText" },
] as const;
...
function Dashboard() {
  ...
  const [stats, setStats] = useState({ avgMoisture: 0, healthy: 0, scans: 0, alerts: 0 });
  const [weather, setWeather] = useState<WeatherResult | null>(null);
  const [pest, setPest] = useState<{ score: number; level: string; trapAvg: number } | null>(null);

  useEffect(() => {
    if (!activeFarm) return;
    void (async () => {
      const [{ data: preds }, { data: dets }, { count: alertCount }] = await Promise.all([
        supabase.from("predictions").select("moisture").eq("farm_id", activeFarm.id).limit(50),
        supabase.from("detections").select("label").eq("farm_id", activeFarm.id).limit(100),
        supabase
          .from("alerts")
  ...
  useEffect(() => {
    ...
    if (!activeFarm?.latitude && typeof navigator !== "undefined" && navigator.geolocation) {
  ...
  const healthPct = stats.scans ? Math.round((stats.healthy / stats.scans) * 100) : 0;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <GlassCard className="animate-rise flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold">
            {t("welcomeBack", { farm: activeFarm?.name ?? t("yourFarm") })}
          </h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        <Stat icon={Droplets} emoji="💧" label={t("statAvgMoisture")} value={`${stats.avgMoisture}%`} note={t("statAvgMoistureNote")} />
        <Stat icon={Leaf} emoji="🌿" label={t("statHealthyPlants")} value={`${stats.healthy}`} note={t("statHealthyPlantsNote", { pct: healthPct, scans: stats.scans })} />
        <Stat icon={AlertTriangle} emoji="⚠️" label={t("statAlerts")} value={`${stats.alerts}`} note={t("statAlertsNote")} />
        <Stat
          icon={Thermometer}
          emoji="🌡️"
          label={t("statTemperature")}
          value={weather ? `${weather.temperature}°C` : "—"}
          note={weather ? t("statTemperatureHumidityNote", { humidity: weather.humidity }) : t("statTemperatureFetching")}
        />
        <Stat
          icon={Bug}
          emoji="🐛"
          ...
          note={pest ? t("statPestPressureNote", { score: pest.score, trapAvg: pest.trapAvg }) : t("statPestPressureEmpty")}
        />
      </div>
            ...
            <GlassCard hover className="animate-rise h-full">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">{t(p.textKey)}</p>
                </div>
